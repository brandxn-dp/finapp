...
export function registerSimplefinRoutes(app: FastifyInstance): void {
  app.get("/api/simplefin/status", async (req) => {
    const hid = req.householdId!;
    return { connected: isConnected(hid), last_sync: lastSync(hid) };
  });
  ...
  app.post("/api/simplefin/claim", async (req, reply) => {
    ...
    try {
      await claimSetupToken(b.token, req.householdId!);
  ...
  app.post("/api/simplefin/sync", async (req, reply) => {
    try {
      return await sync(req.householdId!);
    } catch (err) {
      return reply.code(400).send({ error: err instanceof Error ? err.message : String(err) });
    }
  });

  app.delete("/api/simplefin", async (req) => {
    disconnect(req.householdId!);
